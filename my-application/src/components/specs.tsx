...
const SPECIFICATIONS = [
  {
    ...
    title: 'Free download',
    description: 'Convert and download YouTube videos as much as you want without limitation and always free.',
  },
  {
    icon: <ThumbUpIcon fontSize="large" className='self-center'/>,
    title: 'Fast and easy to use',
    description: 'Just paste the link or type in the search box to get ready to download.',
  },
  {
    icon: <BuildIcon fontSize="large" className='self-center'/>,
    title: 'No registration required',
    description: 'Our downloader is free and you can download any video without user registration.',
  },
  {
        ...
        <div className="p-10 md:p-20 w-full md:w-8/12 md:m-auto grid grid-cols-1 md:grid-cols-2 gap-12 text-center place-content-center border-b border-b-gray-200">
            {SPECIFICATIONS.map((spec) => (
              <div className="flex flex-col text-center" key={spec.title}>
                {spec.icon}
                <div className='text-red-600 font-bold my-2 text-lg md:text-xl'>{spec.title}</div>
